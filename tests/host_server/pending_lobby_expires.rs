...
fn pending_lobby_expires_then_reacks()
{
    ...
    */
    ...
    let Some(HostUserClientEvent::Msg(HostToUserMsg::LobbyState{ lobby })) = user3.next()
    else { panic!("client did not receive server msg"); };
    assert_eq!(lobby.id, made_lobby_id);
    ...
    let Some(HostUserClientEvent::Msg(HostToUserMsg::GameStart{ id: _, token: _, start: _ })) = user3.next()
    else { panic!("client did not receive server msg"); };
