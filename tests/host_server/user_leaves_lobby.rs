...
fn client_leaves_lobby()
{
    ...
    */
    ...
    // user 2 leaves lobby
    user2.request(UserToHostRequest::LeaveLobby{ id: lobby.id }).expect("send failed");
    std::thread::sleep(Duration::from_millis(15));
    host_server.update();
    std::thread::sleep(Duration::from_millis(15));
    ...
    user1.request(UserToHostRequest::LeaveLobby{ id }).expect("send failed");
    std::thread::sleep(Duration::from_millis(15));
    host_server.update();
    std::thread::sleep(Duration::from_millis(15));

    // - users 1, 2 receive lobby leave
    let Some(HostUserClientEvent::Msg(HostToUserMsg::LobbyLeave{ id })) = user1.next()
    else { panic!("client did not receive server msg"); };
    assert_eq!(id, made_lobby_id);
    ...
    // - user 1 receives ack for leaving the lobby
...
fn client_leaves_pending_lobby()
{
    ...
    */
    ...
    // - user 1receives ack for launching the game
    let Some(HostUserClientEvent::Ack(_request_id)) = user1.next()
    else { panic!("client did not receive server msg"); };
    ...
    // - user 2 receives rejection for leave lobby request
    let Some(HostUserClientEvent::Reject(_request_id)) = user2.next()
    else { panic!("client did not receive server msg"); };
...
fn client_resets_pending_lobby()
{
    ...
    */
    ...
    let Some(HostUserClientEvent::Ack(ack_id)) = user1.next()
    else { panic!("client did not receive server msg"); };
