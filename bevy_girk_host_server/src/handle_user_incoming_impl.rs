...
pub(crate) fn user_get_connect_token(
    ...
    let Some((game_id, token)) = ongoing_games_cache.get_user_connect_token(user_id, user_info)
    ...
    user_server.respond(req_token, HostToUserResponse::ConnectToken{ id: game_id, token });
}
